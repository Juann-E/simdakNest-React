...
export default function NamaBarang() {
  ...
  const [units, setUnits] = useState<Unit[]>([]); // State untuk menyimpan daftar satuan
  ...
  // useEffect sekarang akan mengambil data barang dan satuan sekaligus
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Menggunakan Promise.all untuk mengambil kedua data secara paralel
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...formData,
      idSatuan: parseInt(formData.idSatuan, 10), // Pastikan idSatuan adalah angka
    };
    ...
    try {
      ...
      // Ambil ulang semua data untuk memastikan relasi terupdate
      const response = await axios.get('http://localhost:3000/nama-barang', { headers: { Authorization: `Bearer ${token}` } });
